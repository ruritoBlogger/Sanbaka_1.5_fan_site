...
const LizeWindow: React.FC = () => {
  ...
  const MoveToYoutube = () => {
    window.open(
      "https://www.youtube.com/channel/UCZ1xuCK1kNmn5RzPYIZop3w?sub_confirmation=1",
      "_blank"
    );
  };
      ...
      >
        ...
        <div className={styles.paddingRight_pcOnly}>
          ...
          <div className={`${styles.flex_row} ${styles.centerComponent}`}>
            <IconButton
              handleClick={() => MoveToYoutube()}
              link={`${window.location.origin}/image/youtube.png`}
            />
            <IconButton
              handleClick={() => MoveToTwitter()}
              link={`${window.location.origin}/image/twitter.png`}
            />
          </div>
        </div>
      </div>
